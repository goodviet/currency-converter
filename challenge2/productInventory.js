...
console.log(`Total Inventory Value: ${totalValue.toFixed(2)}`); // 10,599.82


const mostExpensiveProduct = inventory.findMostExpensiveProduct();
console.log(`Most Expensive Product: ${mostExpensiveProduct}`); // "Laptop"


const isHeadphonesInStock = inventory.isProductInStock("Headphones");
console.log(`Is Headphones in stock? ${isHeadphonesInStock}`); // false


console.log("Products sorted by price (ascending):");
inventory.sortProductsByPriceAscending().forEach(product => {
    console.log(`${product.name}: $${product.price.toFixed(2)} (${product.quantity} in stock)`);
});

console.log("Products sorted by quantity (descending):");
inventory.sortProductsByQuantityDescending().forEach(product => {
    console.log(`${product.name}: $${product.price.toFixed(2)} (${product.quantity} in stock)`);
});
